import sys
from C3_SemiSeg.base.base_model import BaseModel
import torch
import math
...
from C3_SemiSeg.utils.helper import initialize_weights
from C3_SemiSeg.utils.checkpoint import load_state_dict
from itertools import chain
from C3_SemiSeg.models.pretrain import PRETRAIN_PATH
import pdb
...
class ResNet(nn.Module):
    def __init__(self, in_channels=3, output_stride=16, backbone='resnet50', pretrained=True):
        ...
        pretrain_path = PRETRAIN_PATH['resnet50']
        # pretrain_path = pretrain_path.replace('../pretrains', '/cache/pretrains')
        model = getattr(models, backbone)(False)
...
class ASSP(nn.Module):
    def __init__(self, in_channels, output_stride):
        ...
        self.avg_pool = nn.Sequential(
            ...
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            # update 06/11, change order: avgpool->conv->bn->relu to conv->bn->relu>avg_pool
        )
...
class DeepLabV3(BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet50', pretrained=True,
                 output_stride=16, freeze_bn=False, return_feat=False, return_feat_and_logit=False, **_):
        ...
        self.return_feat = return_feat and not return_feat_and_logit
    ...
    def forward(self, x):
        ...
        feat, low_level_features = self.backbone(x)
        x = self.ASSP(feat)
...
if __name__ == '__main__':
    model = DeepLabV3(19, backbone='resnet', pretrained=True)
    pdb.set_trace()
    inputs = torch.zeros((1, 3, 256, 512)).cuda()
    out = model(inputs)