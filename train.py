...
def train(num_folds=6, num_epochs=50, batch_size=8, supervised_loss='cross_entropy', contrastive_loss='pixel'):
    num_classes = 3
    num_folds = num_folds
    ...
    if contrastive_loss == 'pixel':
        ...
        contrastive_loss_fn = losses.HybridContrastiveLoss(
            temperature=0.1, neighborhood_size=5, weight_pixel=1.0, weight_local=1.0, weight_directional=1.0
        )
    else:
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(range(train_dataset_size))):
        ...
        for epoch in range(num_epochs):
            ...
            try:
                pseudo_labels = generate_pseudo_labels(model, unlabeled_dataloader, device=device)
            ...
            for images, pseudo_labels in tqdm(unlabeled_dataloader, desc=f"Epoch {epoch + 1}/{num_epochs} - Semi-supervised"):
        ...
        with torch.no_grad():
            for images, labels in tqdm(val_dataloader, desc=f"Validation Fold {fold + 1}"):
                ...
                for pred, label in zip(preds, labels):
                    iou_scores.append(
                        jaccard_score(label.cpu().numpy().flatten(), pred.cpu().numpy().flatten(), average='binary'))
